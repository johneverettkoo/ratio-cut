...
\sum_i^{r-1} (\prod_{j \neq i} \lambda_i) (\lambda_i - \lambda_r) (p_i - q_i)
  \leq 0$$
...
* $(H^\top L H)^\dagger = c (H^\top L^\dagger H)$ for some positive constant c. 
Similarly, $(H^\top L^\dagger H)^\dagger = c' (H^\top L H)$. So 
$\arg\min_H \tr(H^\top L H) = \arg\min_H \tr((H^\top L^\dagger H)^\dagger)$ and 
$\arg\max_H \tr(H^\top L^\dagger H) = \arg\max_H \tr((H^\top L H)^\dagger)$.
